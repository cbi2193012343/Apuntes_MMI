...
\begin{frame}{Puntos fijos}
    ...
    \end{tikzpicture}
    \caption{Punto fijo $p$ como intersecci\'on de $y=f(x)$ con la recta $y=x$.}
    \end{figure}
\end{frame}
...
\begin{frame}[fragile]{Diagrama de bifurcaci\'on log\'istico}
    \begin{columns}[c]
        \column{0.56\textwidth}
            \begin{figure}
                \centering
                \includegraphics[height=0.68\textheight]{logistic-map-fig/logistic_map.png}
                \caption{Valores iterados de $F_\mu$ tras descartar el transitorio, $2.5\leq\mu\leq4$.}
            \end{figure}
...
\begin{frame}{La funci\'on $T_\mu(x)=\mu\min(x,1-x)$}
    \begin{figure}
        \centering
        \includegraphics[height=0.70\textheight]{tent-map-fig/tent_function.png}
        \caption{Gr\'aficas de $T_\mu$ para $\mu=1$, $\mu=1.5$ y $\mu=2$.}
    \end{figure}
\end{frame}
...
\begin{frame}[fragile]{Diagrama de bifurcaci\'on del map tent}
    \begin{columns}[c]
        \column{0.56\textwidth}
            \begin{figure}
                \centering
                \includegraphics[height=0.68\textheight]{tent-map-fig/tent_map.png}
                \caption{Estados finales de $T_\mu$ tras $1000$ iteraciones, para $2000$ condiciones iniciales por par\'ametro.}
            \end{figure}
        \column{0.44\textwidth}
